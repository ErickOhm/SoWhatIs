...
type ComponentChildren = { children: ReactNode };
...
export default function Tabs({ sharedStore, ...slots }: Props) {
	...
	const text_base = "var(--color-text-base)";
	const color_card = "var(--color-card)";

	return (
		<div className={styles.container}>
			<div className={`${styles["tab-scroll-overflow"]} tablistContainer`}>
				<div
				...
				>
					{tabs.map(([key, content]) => (
						<button
							...
							role="tab"
							type="button"
							className={`${styles.tab} text-white`}
							id={`${tabId}-${key}`}
							key={key}
						>
							{content}
